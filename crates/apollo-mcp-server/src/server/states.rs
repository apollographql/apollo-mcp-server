...
struct Config {
    ...
    search_flatten_depth: usize,
    semantic_enabled: bool,
    semantic_model: String,
    semantic_inference_threads: usize,
    semantic_cache_path: Option<std::path::PathBuf>,
    hybrid_rrf_k: f32,
    ...
    pub(crate) async fn start(self, server: Server) -> Result<ShutdownReason, ServerError> {
        ...
        let mut state = State::Configuring(Configuring {
            config: Config {
                ...
                search_default_limit: server.search_default_limit,
                search_max_limit: server.search_max_limit,
                search_flatten_depth: server.search_flatten_depth,
                semantic_enabled: server.semantic_enabled,
                semantic_model: server.semantic_model,
                semantic_inference_threads: server.semantic_inference_threads,
                semantic_cache_path: server.semantic_cache_path,
                hybrid_rrf_k: server.hybrid_rrf_k,
