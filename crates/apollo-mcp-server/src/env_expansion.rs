...
mod tests {
    ...
    #[test]
    fn expand_yaml_coerces_boolean() {
    ...
    #[test]
    fn expand_yaml_preserves_string() {
        figment::Jail::expect_with(|jail| {
    ...
    #[test]
    fn expand_yaml_handles_special_chars_safely() {
    ...
    #[test]
    fn expand_yaml_expands_nested_structures() {
        figment::Jail::expect_with(|jail| {
...
"#;
    ...
    #[test]
    fn expand_yaml_unquoted_also_coerces() {
        // Unquoted env vars should also work (parsed as strings by YAML first)
        figment::Jail::expect_with(|jail| {
            jail.set_env("MY_NUM", "123");
            // Note: unquoted ${env.X} is parsed as a string by serde_yaml
    ...
    #[test]
    fn coerce_null_to_null() {
        assert!(coerce("null").is_null());
    }

    #[test]
    fn coerce_mapping_stays_string() {
        // "key: value" would parse as a mapping, but we want it as a string
        let result = coerce("key: value");
        assert_eq!(result, Value::String("key: value".to_string()));
    }

    #[test]
    fn coerce_sequence_stays_string() {
        let result = coerce("[1, 2, 3]");
        assert_eq!(result, Value::String("[1, 2, 3]".to_string()));
    }

    // =========================================================================
    // Tests for expand_env_vars (string-level expansion)
    // =========================================================================
