...
mod tests {
    ...
    mod oauth_validate {
        ...
        /// End-to-end check that a valid token lacking a required scope is
        /// rejected with `403` by the middleware. A mock authorization server
        /// serves OIDC discovery and a symmetric JWKS so the token actually
        /// validates, then scope enforcement rejects it.
        #[tokio::test]
