...
mod test {
    ...
    #[test]
    fn should_handle_inline_fragments_on_union_types() {
        let source_text = r#"
            type Query {
                entities: [Entity]
            }

            union Entity = Event | Session | Place
        ...
        let (operation_document, operation_def, _comments) = operation_defs(
            "query GetEntities { \
                entities { \
                    ... on Event { name year startDate endDate city } \
                    ... on Session { name startTimeTs } \
                    ... on Place { name company displayName location primaryTypeDisplayName } \
                }
            }",
            false,
            Some("operation.graphql".to_string()),
        ...
        let shaken_schema = shaker.shaken().unwrap();
        let event_type = shaken_schema.types.get("Event");
        assert!(event_type.is_some());

        let session_type = shaken_schema.types.get("Session");
        assert!(session_type.is_some());

        let place_type = shaken_schema.types.get("Place");
        assert!(place_type.is_some());
    }

    #[test]
    fn should_handle_named_fragments() {
        let source_text = r#"
            type Query {
                entities: [Entity]
            }

            union Entity = Event | Session
