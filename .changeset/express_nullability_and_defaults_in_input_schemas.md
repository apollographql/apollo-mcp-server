...
The shape of generated tool input schemas changes: every nullable variable, input object field, and list item is now wrapped as `{"oneOf": [<type>, {"type": "null"}]}`, the same shape output schemas already use. Any call that was valid before stays valid, since omitting a property is still allowed and `required` only shrinks. Hosts that inspect `properties.<name>.type` directly will see `oneOf` instead of a bare type, and tests that snapshot tool schemas will need updating.
