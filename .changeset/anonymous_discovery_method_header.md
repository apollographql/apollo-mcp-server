...
The deprecated `allow_anonymous_mcp_discovery` flag now also permits `server/discover`, preserving its existing anonymous access to `initialize`, `tools/list`, and `resources/list`. A local initialization check rejects contradictory method headers to cover rmcp's handshake exemption.
