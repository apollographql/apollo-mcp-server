...
This protection only takes effect once `auth.issuers` or `auth.audiences` is configured; with neither set (i.e. `issuers: []` and `allow_any_audience: true`), the gate is a pass-through by design.
