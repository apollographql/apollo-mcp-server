...
In order to provide specific `toolInput` types, you must configure a `schema` with the `apolloClientAiApps` Vite plugin. Refer to the [Vite configuration](#vite-configuration) section to learn more.
...
You might be tempted to reach for `useToolInfo` to populate the initial values for these variables, but this approach quickly reveals additional complexity that `useToolInfo` can't handle on its own:

- The tool called by the agent might not be the same tool
- When a user navigates away from the screen and back again, the input might be different than the original tool input
- Some variables might need to sync with an external source, such as `props`
- Some variables might change as a result of user interaction

The `createHydrationUtils` utility exists to bridge this gap. Call `createHydrationUtils` with the query that provides the tool definition. This returns a `useHydratedVariables` hook used manage variables for the query. The returned `variables` are provided to the `useQuery` hook in the `variables` option.
...
In some cases, you might want to alter a value in your application depending on the running platform. The `Platform` utility provides a straightforward way to detect the current platform and provide platform-specific values at runtime.
...
`apolloClientAiApps` generates TypeScript types for tools defined in your application by the `@tool` directive, including tool names and inputs. In order for input types to resolve correctly, the Vite plugin must know about inputs types defined by your GraphQL schema. You can configure a schema using the `schema` option.
...
The `apolloClientAiApps` Vite plugin uses [GraphQL Codegen](https://the-guild.dev/graphql/codegen) under-the-hood. The `schema` option accepts any value that [GraphQL Codegen accepts](https://the-guild.dev/graphql/codegen/docs/config-reference/schema-field#available-formats).
