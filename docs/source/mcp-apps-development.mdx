...
Initialize an instance of `ApolloClient` from the `@apollo/client-ai-apps` package. The `ApolloClient` constructor needs at least an instance of an `ApolloCache` and the [manifest file](./mcp-apps-reference#manifest-file).
