...
`toolName` and `toolInput` are typed according to the tools defined in your application. Refer to the [typescript configuration](#typescript-configuration) section to learn how to include the generated types are included in your application.
...
`useHydratedVariables` compares the configured query to the tool called by the agent to determine if they match. If they match, `variables` returned by the hook is set to the value of the tool input provided by the LLM. If they don't match, the variables object provided as an argument to `useHydratedVariables` is used instead.

<Note>

The `variables` value returned by `useHydratedVariables` only includes keys for variables defined in the query and does not include additional input values. For example, `@tool` includes an `extraInputs` argument used to provide additional tool input. These values are stripped away in the returned `variables` value.
...
To ensure your UI reflects the accurate state of variables, read the values from the `variables` returned by `useHydratedVariables`. This ensures
...
Marking a variable `reactive` forbids you from using that variable in the `setVariables` function. This avoids accidental bugs that might keep that value out-of-sync. If a reactive variable is updated, a warning is logged and the update is ignored.

<Note>

When a reactive variable does not match tool input, the tool input value is used. The value of that variable will remain until the external value changes. When it changes, the new value is used moving forward.
...
export default defineConfig({
  plugins: [
    apolloClientAiApps({
      targets: ["mcp", "openai"],
      appsOutDir: "../apps/my-app", // path where build artifacts are written
    }),
    // ... other plugins
  ],
  // ... other config
...
The `appsOutDir` option controls where the build artifacts (including the manifest file and built resources) are written. This should point to the corresponding directory under `apps/` that Apollo MCP Server reads from.
