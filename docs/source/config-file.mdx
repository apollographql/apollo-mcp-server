...
| `metadata`    | `Key/Value Pairs`   |                         | The metadata key values for GRPC protocol                                                                                                                                                               |
| `headers`     | `Key/Value Pairs`   |                         | The header key values for HTTP/Protobuf protocol                                                                                                                                                        |

<ExpansionPanel title="Metrics telemetry config example using gRPC">

The following example config section sets the metrics telemetry to endpoint `http://localhost:4317`, using the gRPC protocol, with a cumulative temporality, and some metadata values to send with the gRPC requests.
...
| `metadata`   | Key-value pairs                              |                         | The metadata key-value pairs. Used only if `protocol` is set to `grpc`.                      |
| `headers`    | Key-value pairs                              |                         | The header key-value pairs. Used only if `protocol` is set to `http/protobuf`.               |

<ExpansionPanel title="Tracing telemetry config example using HTTP/Protobuf">

The following example config section sets the tracing telemetry to endpoint `http://localhost:4317`, using the `http/protobuf` protocol, and sets some header values via the `headers` field (instead of `metadata`).
