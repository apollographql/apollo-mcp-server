...
Apollo MCP Server redacts the GraphOS key from debug configuration output. This does not encrypt the key or prevent it from being written literally in YAML. Use the `APOLLO_KEY` environment variable or another runtime secret-injection mechanism.
...
At `debug` level, Apollo MCP Server logs the parsed configuration. Ordinary static header values, including values you populate through `${env.VAR}`, can appear in this output. The GraphOS key is redacted. Do not enable debug logging in production without protecting and restricting access to the resulting logs.
...
Environment variable expansion applies only to the YAML configuration file. Local `.graphql` operation files and persisted query manifest bodies are loaded exactly as written. Do not embed credentials in operation content or expect `${env.VAR}` placeholders there to be expanded.
...
A successfully fetched source update replaces the current catalog; it isn't compared with an approved version or held for confirmation. Operations that Apollo MCP Server can't parse are skipped during the rebuild instead of preserving their previous versions. For deployment guidance, see [Protect tool definitions from poisoning](/apollo-mcp-server/best-practices#protect-tool-definitions-from-poisoning).
...
| `required_scopes`            | `Map<String, List<String>>`         | `{}`     | Additional OAuth scopes for listed operations, keyed by exact MCP tool name. Requirements add to the global scope requirement and use all-of semantics. Unlisted tools receive no additional restriction. [Learn more about per-operation scope requirements](/apollo-mcp-server/auth#per-operation-scope-requirements). |

#### Mutation modes

`mutation_mode` applies to predefined GraphQL operations loaded from every configured operation source. It also controls whether an enabled [`execute` tool](/apollo-mcp-server/define-tools#introspection-tools) accepts an ad hoc mutation.
...
`mutation_mode` is a configuration gate, not a per-invocation approval mechanism. `explicit` means the mutation is already present in the active operation source; it doesn't mean the operation came from a local file, was committed to version control, or passed a review enforced by Apollo MCP Server. Hot-reloaded source changes can therefore add or change mutations without an approval step.

The setting classifies documents by their declared GraphQL operation type. It cannot detect side effects implemented behind a query field, so ensure your upstream GraphQL API preserves read-only query semantics.
...
Apollo MCP Server parses operation documents locally but doesn't automatically validate them against the configured schema. See [Operation validation and execution](/apollo-mcp-server/limitations#operation-validation-and-execution).
...
| `disable_auth_token_passthrough`  | `bool`                | `false`       | Disable automatic propagation of the validated Authorization header to the downstream API. Disabling it leaves headers forwarded via `forward_headers` untouched; when enabled, the validated token replaces a forwarded `Authorization` header. |
...
Expansion applies only to values in the YAML configuration file. Apollo MCP Server reads local `.graphql` operation files and persisted query manifest bodies verbatim. Do not embed credentials in operation content or expect `${env.VAR}` placeholders to expand.
