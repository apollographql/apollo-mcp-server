...
| `allow_anonymous_mcp_discovery`   | `bool`                | `false`       | **Deprecated.** Use `skip_token_validation.methods` instead. Enabling this is the same as listing `server/discover`, `tools/list`, and `resources/list` there. Setting both is an error.           |
