...
| Manifest           | `descriptions` | `Map<String, String>`     | Optional map from operation name to tool description. Overrides auto-generated descriptions. [Learn more](/apollo-mcp-server/define-tools#tool-descriptions-for-manifest-operations). |
...
<Note>
When using `source: collection` or `source: uplink`, the MCP Server polls GraphOS for changes periodically. Expect up to **30–60 seconds** before new or modified operations appear as tools.
</Note>
