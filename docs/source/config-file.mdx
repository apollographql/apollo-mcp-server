...
| Option                            | Type                  | Default       | Description                                                                                                                                                                                                      |
| :-------------------------------- | :-------------------- | :------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `servers`                         | `List<URL>`           |               | List of upstream delegated OAuth servers (must support RFC 8414 or OIDC metadata discovery endpoint). Required; set to `[]` if every server is listed under `static_servers` instead.                            |
| `static_servers`                  | `List<Object>`         | `[]`          | Upstream OAuth servers without a discovery endpoint. Each entry sets `issuer` and `jwks_uri` directly. See [authorization servers without discovery](/apollo-mcp-server/auth#authorization-servers-without-discovery). |
