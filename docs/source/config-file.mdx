...
| `issuers`                         | `List<string>`        | `[]`          | Additional allowlist for token issuers. When set, `iss` must match this list and the issuer advertised by the signing server. Empty skips token issuer checks, not authorization-server metadata validation.     |
...
transport:
  ...
  auth:
    ...
    # of these values and the issuer the signing server advertises in discovery.
