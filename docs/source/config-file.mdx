...
| `format`   | `oneOf ["full", "compact", "json", "pretty"]`       | `"full"`   | The log output format style                                                       |
...
The following example configures metrics telemetry to use the `grpc` protocol and sends data to the `http://localhost:4317` endpoint. It uses a cumulative temporality and includes metadata with each request.
