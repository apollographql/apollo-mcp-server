...
| `descriptions`               | `Map<String, String>`               | `{}`     | Optional map from operation name to tool description. Overrides auto-generated descriptions for any operation source. See [config-level descriptions](/apollo-mcp-server/define-tools#config-level-descriptions). |
...
| `required_scopes`            | `Map<String, List<String> \| List<List<String>>>` | `{}`     | Optional map from operation name to required OAuth scopes. A flat list requires all scopes. Nested lists allow alternatives: each inner list is AND, and the outer list is OR. When a token lacks the required scopes for an operation, Apollo MCP Server returns `403 Forbidden` with `WWW-Authenticate: Bearer error="insufficient_scope"`. See [per-operation scope requirements](/apollo-mcp-server/auth#per-operation-scope-requirements). |
