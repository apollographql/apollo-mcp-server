...
| `tls.ca_cert`                    | `string`       |         | Path to additional CA certificates to trust (PEM format). |
| `tls.danger_accept_invalid_certs`| `bool`         | `false` | Accept invalid TLS certificates. **Warning**: Insecure—use only for development/testing.          |
...
transport:
  ...
  auth:
    ...
    tls:
      ...
      # Only set to true for development/testing (insecure!)
