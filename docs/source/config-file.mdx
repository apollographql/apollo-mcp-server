...
| `caching.ttl_ms`             | `u64`                               | `300000` | Freshness lifetime in milliseconds for cache hints on `tools/list`, `resources/list`, `resources/read`, and `prompts/list` responses. Cache scope is always `private`. Requires negotiation of MCP protocol version `2026-07-28` or newer; see the availability note below. |

Cache hints are currently inactive: this server negotiates MCP protocol versions up to `2025-11-25`. The `caching.ttl_ms` setting is reserved for protocol support in a future release and does not currently change client caching behavior.
