...
Additional allowlist for token issuers. When set, `iss` must match this list and the issuer advertised by the signing server. Empty skips token issuer checks, not authorization-server metadata validation.
...
Disable automatic propagation of the validated Authorization header to the downstream API. This doesn't override `forward_headers`.
