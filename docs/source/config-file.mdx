...
The server watches the configuration file for changes. When the file is modified, the server automatically restarts with the updated configuration. For `streamable_http` transport, the restart happens in-place. For `stdio` transport, the process exits with code 75 so the parent process can restart it.

<Note>

Changes to `logging` and `telemetry` configuration are not applied during in-process hot-reload (`streamable_http` transport). These settings are initialized once at startup and require a full process restart to take effect.
...
Apollo MCP Server redacts the GraphOS key from debug configuration output. This doesn't encrypt the key or prevent it from being written literally in YAML. Use the `APOLLO_KEY` environment variable or another runtime secret-injection mechanism.
...
Static header values are ordinary strings after the configuration loads and are not redacted from debug configuration output. Using `${env.VAR}` keeps the literal value out of the YAML file, but it does not redact the expanded value at runtime. Unlike static headers, the GraphOS key is redacted from debug output.
...
With `transport.auth` configured, the HTTP authentication middleware validates the request before a tool call reaches the GraphQL API. Use an auth block to ensure Apollo MCP Server validates an explicitly forwarded `authorization` header.
...
`disable_auth_token_passthrough` disables only the automatic propagation of a validated OAuth token. If `forward_headers` contains `Authorization`, Apollo MCP Server still copies the incoming header to the GraphQL request even when passthrough is disabled. When passthrough is enabled, the validated token replaces a forwarded `Authorization` header. Audit both settings together.

<br />
<br />

According to [MCP security best practices](https://modelcontextprotocol.io/specification/2025-11-25/basic/security_best_practices#token-passthrough) and the [MCP authorization specification](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization#access-token-privilege-restriction), token passthrough introduces serious security risks:
...
| `allow_headers`     | `List<string>` | `["content-type", "mcp-protocol-version", "mcp-session-id", "traceparent", "tracestate", "baggage"]` | Request headers enabled for cross-origin requests, including MCP, W3C Trace Context, and W3C Baggage headers                   |
...
| `execute.enabled`           | `bool`   | `false`    | Expose the `execute` tool for ad hoc GraphQL operations                |
...
The default value for `source` is `"infer"`. See [defining tools as operations](/apollo-mcp-server/define-tools).

| Source             | Option   | Type             | Description                                                                                                                                             |
| :----------------- | :------- | :--------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------ |
| GraphOS Collection | `source` | `"collection"`   | Load operations from a GraphOS collection                                                                                                               |
| GraphOS Collection | `id`     | `string`         | The collection ID to use in GraphOS. Use `default` for the default collection. See [GraphOS operation collections](/apollo-mcp-server/define-tools#from-operation-collection). |
...
Environment variable expansion applies only to the YAML configuration file. Local `.graphql` operation files and persisted query manifest bodies are loaded exactly as written. Don't embed credentials in operation content or expect `${env.VAR}` placeholders to be expanded.
...
Operation collection polling supports collections containing up to 100 operations. The server loads a larger collection, but subsequent polling is disabled. If an update increases a collection beyond 100 operations, the server applies that update and polling then stops.

</Note>

For `source: collection`, the server retries connection failures, timeouts, HTTP `429` responses, and server errors.

A successfully fetched source update replaces the current catalog; it isn't compared with an approved version or held for confirmation. Operations that Apollo MCP Server can't parse are skipped during the rebuild instead of preserving their previous versions. For deployment guidance, go to [Protect tool definitions from poisoning](/apollo-mcp-server/best-practices#protect-tool-definitions-from-poisoning).
...
These fields are under the top-level `caching` key. Set `APOLLO_MCP_CACHING__TTL_MS` to override the TTL through an environment variable.

| Option | Type | Default | Description |
| :----- | :--- | :------ | :---------- |
| `ttl_ms` | `u64` | `300000` | Freshness lifetime in milliseconds for cache hints on `tools/list`, `resources/list`, `resources/read`, and `prompts/list` responses. Remote app resource reads omit cache hints. Cache scope is always `private` when hints are provided. Requires negotiation of MCP protocol version `2026-07-28` or newer; see the following availability note. |

<Note>

Cache hints are currently inactive: Apollo MCP Server negotiates MCP protocol versions up to `2025-11-25`. The `caching.ttl_ms` setting doesn't currently change client caching behavior because the required protocol revision isn't supported.
...
| `mutation_mode`              | `oneOf ["none", "explicit", "all"]` | `"none"` | Controls which GraphQL mutation operation types the MCP server exposes                                                                                                                          |
| `descriptions`               | `Map<String, String>`               | `{}`     | Top-level tool descriptions keyed by exact operation name. Overrides source-derived descriptions for matching operations but doesn't affect input descriptions or executable documents. Unmatched keys don't produce a warning. [Learn more about config-level descriptions](/apollo-mcp-server/define-tools#config-level-descriptions). |
...
| `required_scopes`            | `Map<String, List<String> \| List<List<String>>>` | `{}`     | Additional OAuth scopes for listed operations, keyed by exact MCP tool name. Unlisted tools receive no additional restriction. A flat list requires all scopes; nested lists define alternatives, where each inner list is AND and the outer list is OR. Requirements add to the global scope requirement. When a token lacks the required scopes for an operation, the server returns HTTP 403 with `WWW-Authenticate: Bearer error="insufficient_scope"`. [Learn more about per-operation scope requirements](/apollo-mcp-server/auth#per-operation-scope-requirements). |

#### Mutation modes

`mutation_mode` applies to predefined GraphQL operations loaded from every configured operation source. `mutation_mode` also controls whether an enabled [`execute` tool](/apollo-mcp-server/define-tools#introspection-tools) accepts an ad hoc mutation.

| Mode       | Predefined queries | Predefined mutations | Ad hoc queries through `execute` | Ad hoc mutations through `execute` |
| :--------- | :----------------- | :------------------- | :------------------------------- | :--------------------------------- |
| `none`     | Exposed            | Skipped              | Allowed                          | Rejected                           |
| `explicit` | Exposed            | Exposed              | Allowed                          | Rejected                           |
| `all`      | Exposed            | Exposed              | Allowed                          | Allowed                            |

The ad hoc columns apply only when `introspection.execute.enabled` is `true`. The server never exposes subscriptions as predefined tools and `execute` rejects them in every mode.

<Caution>

`mutation_mode` is a configuration gate, not a per-invocation approval mechanism. `explicit` means the mutation is already present in the active operation source; it doesn't mean the operation came from a local file, was committed to version control, or passed a review enforced by Apollo MCP Server. As a result, hot-reloaded source changes can add or change mutations without an approval step.

The setting classifies documents by their declared GraphQL operation type. It can't detect side effects implemented behind a query field, so make sure your upstream GraphQL API preserves read-only query semantics.

</Caution>

<Note>

MCP App manifests are subject to the same mutation gate. With `mutation_mode: none`, Apollo MCP Server rejects an app manifest that contains a mutation instead of partially loading that app.

</Note>

Apollo MCP Server parses operation documents locally but doesn't automatically validate them against the configured schema. For more information, go to [Operation validation and execution](/apollo-mcp-server/limitations#operation-validation-and-execution).
...
For `source: graphos`, transient errors before the initial schema load are retried, although a permanent initial error prevents startup. After a schema has loaded, later poll or fetch errors keep the current schema and polling continues. This last-known-good behavior applies to request failures, not to the contents of a successfully fetched publication: a schema document that can't be parsed and validated causes the server to exit with an error.
...
| Option            | Type           | Default | Description                                                                 |
| :---------------- | :------------- | :------ | :-------------------------------------------------------------------------- |
| `enabled`         | `bool`         | `true`  | Use Host validation and any configured Origin validation                 |
...
Authentication is available only for `streamable_http`. Because the default transport is `stdio`, you must also set `transport.type: streamable_http` in your auth configuration. The server rejects an `auth` block configured under `stdio`.
...
| `issuers`                         | `List<string>`        | `[]`          | Additional allowlist for token issuers. When set, `iss` must match this list and the issuer advertised in the authorization server's discovery metadata. Empty skips token issuer checks, not authorization-server metadata validation.     |
...
| `scopes`                          | `List<string>`        | `[]`          | Global OAuth scopes supported by this MCP resource server. The server advertises them to clients and enforces them according to `scope_mode`. Empty imposes no global scope requirement.                        |
...
transport:
  ...
  auth:
    ...
    # Additional allowlist for token issuers. The JWT `iss` claim must match one
    # of these values and the issuer the authorization server advertises in discovery.
    # When empty (the default), the server skips these token issuer checks.
    issuers:
      - "https://auth.example.com"
    ...
    # Global OAuth scopes supported by this MCP resource server.
    # These are enforced according to `scope_mode`.
    scopes:
      - read
      - mcp
      - profile
    ...
    # Requests that skip token validation. Each list applies only when the
    # request carries no Authorization header. Every other request is validated.
    # skip_token_validation:
    #   methods:
    #     - tools/list
    #     - server/discover
...
| `omitted_attributes` | `List<String>`         |                             | Supported Apollo metric attributes to omit before export.                                                 |
...
| `omitted_attributes` | `List<String>`        |                             | Supported Apollo span attributes to omit before export. |
...
| Option      | Type       | Description                                                                                   |
| :---------- | :--------- | :-------------------------------------------------------------------------------------------- |
| `src`       | `string`   | Parseable URI of the icon, such as an HTTP or HTTPS URL or a base64-encoded `data:` URI. Required. |
| `mime_type` | `string`   | Explicit MIME type such as `image/png` or `image/svg+xml`. Use `mime_type` in YAML; `mimeType` is not accepted. Optional. |
| `sizes`     | `string[]` | Icon size strings. Clients expect `<WIDTH>x<HEIGHT>` (for example, `48x48`) or `any` for scalable formats. The server does not enforce these formats. Optional. |
...
Expansion applies only to values in the YAML configuration file. Apollo MCP Server reads local `.graphql` operation files and persisted query manifest bodies verbatim. Don't embed credentials in operation content or expect `${env.VAR}` placeholders to expand.
