...
| `caching.ttl_ms`             | `u64`                               | `300000` | Freshness lifetime in milliseconds for cache hints on `tools/list`, `resources/list`, `resources/read`, and `prompts/list` responses. Cache scope is always `private`. Requires negotiation of MCP protocol version `2026-07-28` or newer; see the following availability note. |
