...
The following example config section sets the telemetry exporter to endpoint `http://localhost:4317`, using the gRPC protocol, with a cumulative temporality, and some metadata values to send with the gRPC requests.
...
The following example config section is similar to above but uses the `http/protobuf` protocol along with the `headers` field (instead of `metadata`).
