...
| `annotations`                | `Map<String, AnnotationOverrides>`  | `{}`     | Optional map from operation name to MCP tool annotation hints. Merges with auto-detected annotations. [Learn more](/apollo-mcp-server/define-tools#config-level-annotations). |
