...
| `allow_anonymous_mcp_discovery`   | `bool`                | `false`       | Allow unauthenticated access to MCP discovery methods (`initialize`, `server/discover`, `tools/list`, `resources/list`). See [anonymous MCP discovery](/apollo-mcp-server/auth#anonymous-mcp-discovery).                            |
...
transport:
  ...
  auth:
    ...
    # (initialize, server/discover, tools/list, resources/list). All other methods still require auth.
