...
Apollo MCP Server redacts the GraphOS key from debug configuration output. This does not encrypt the key or prevent it from being written literally in YAML. Prefer the `APOLLO_KEY` environment variable or another runtime secret-injection mechanism.
...
Static header values are ordinary strings after the configuration is loaded and are not redacted from debug configuration output. Using `${env.VAR}` keeps the literal value out of the YAML file, but it does not redact the expanded value at runtime. Unlike static headers, the GraphOS key is redacted from debug output.
...
With `transport.auth` configured, the HTTP authentication middleware validates the request before a tool call reaches the GraphQL API. Without an auth block, Apollo MCP Server does not validate an explicitly forwarded `authorization` header.
...
`disable_auth_token_passthrough` disables only the automatic propagation of a validated OAuth token. It does not override `forward_headers`. If `forward_headers` contains `authorization`, Apollo MCP Server still copies the incoming header to the GraphQL request. Audit both settings together.
...
At `debug` level, Apollo MCP Server logs the parsed configuration. Ordinary static header values, including values populated through `${env.VAR}`, can appear in this output. The GraphOS key is redacted. Do not enable debug logging in production without protecting and restricting access to the resulting logs.
...
Environment variable expansion applies only to the YAML configuration file. Local `.graphql` operation files and persisted query manifest bodies are loaded verbatim. Do not embed credentials in operation content or expect `${env.VAR}` placeholders there to be expanded.
...
Operation collection polling supports collections containing up to 100 operations. A larger collection is loaded, but subsequent polling is disabled. If an update increases a collection beyond 100 operations, that update is applied and polling then stops.

</Note>

For `source: collection`, connection failures, timeouts, HTTP `429` responses, and server errors are retried. A permanent error before the first successful load prevents the server from finishing startup. After a successful load, later poll or fetch errors keep the current operation catalog while polling continues.

A successfully fetched source update replaces the current catalog; it isn't compared with an approved version or held for confirmation. Operations that Apollo MCP Server can't parse are skipped during the rebuild instead of preserving their previous versions. See [Protect tool definitions from poisoning](/apollo-mcp-server/best-practices#protect-tool-definitions-from-poisoning) for deployment guidance.
...
The ad hoc columns apply only when `introspection.execute.enabled` is `true`. Subscriptions are never exposed as predefined tools and are rejected by `execute` in every mode.
...
The setting classifies documents by their declared GraphQL operation type. It cannot detect side effects implemented behind a `query` field, so the upstream GraphQL API must preserve read-only query semantics.
...
| Option            | Type           | Default | Description                                                                 |
| :---------------- | :------------- | :------ | :-------------------------------------------------------------------------- |
| `enabled`         | `bool`         | `true`  | Enable Host validation and any configured Origin validation                 |
...
Authentication is available only for `streamable_http`. Because the default transport is `stdio`, an auth configuration must also set `transport.type: streamable_http`. The server rejects an `auth` block configured under `stdio`.
...
| `disable_auth_token_passthrough`  | `bool`                | `false`       | Disable automatic propagation of the validated Authorization header to the downstream API. This does not override `forward_headers`.                                                                             |
...
Expansion applies only to values in the YAML configuration file. Apollo MCP Server reads local `.graphql` operation files and persisted query manifest bodies verbatim. Do not embed credentials in operation content or expect `${env.VAR}` placeholders there to be expanded.
