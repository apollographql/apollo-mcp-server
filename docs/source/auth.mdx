...
You can control how the MCP Server validates scopes from OAuth tokens using the `scope_mode` option.
...
Use `scope_mode: disabled` only when downstream services (e.g., subgraphs) handle authorization. Without scope enforcement, any valid token grants full access to the MCP Server.
