...
The flat list syntax requires every listed scope for the operation. In the example above, `DeleteUser` requires both `user:write` and `admin`.
...
In the example above, `UpdateUser` requires either (`user:write` and `tenant:admin`) or `admin`.

When a tool call is made and the token is missing the required scopes, the server returns HTTP 403 with a `WWW-Authenticate` header:

```
WWW-Authenticate: Bearer error="insufficient_scope", scope="user:write admin"
```

The client can use this response to initiate a targeted re-authorization and retry. The OAuth `scope` auth-param is a space-delimited list and can't represent grouped OR conditions, so for nested alternatives the header includes the complete alternative that requires the fewest additional scopes for the presented token. Empty inner alternatives are rejected during config parsing. If auth is not configured or the server is running in stdio mode, `required_scopes` is silently ignored.
