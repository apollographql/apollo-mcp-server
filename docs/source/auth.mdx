...
The client can use this response to initiate targeted re-authorization and retry. The OAuth `scope` auth-param is a space-delimited list and cannot represent grouped OR conditions. For nested alternatives, the header includes the complete alternative that requires the fewest additional scopes for the presented token; if multiple alternatives are tied, Apollo MCP Server includes the first one listed. Apollo MCP Server rejects an empty scope list and empty inner alternatives during config parsing, so a `required_scopes` entry can never be accidentally satisfied by every token. When auth is not configured or Apollo MCP Server runs in stdio mode, Apollo MCP Server skips `required_scopes`.
