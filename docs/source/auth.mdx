...
**Considerations:**
- Discovery happens once at startup when validating the first JWT token
...
- Discovery results are cached by the underlying JWKS library, so subsequent token validations are fast