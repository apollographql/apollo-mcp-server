...
Adopt `scope_mode: disabled` when downstream services, like subgraphs, handle authorization or when you rely exclusively on per-operation requirements. Without a matching per-operation requirement, the MCP Server permits any valid token to call the tool.
