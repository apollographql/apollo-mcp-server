...
For requests declaring an SDK-known `MCP-Protocol-Version` of `2026-07-28` or later, the server checks `Mcp-Method` before reading the body. A matching method skips auth middleware buffering; a nonmatching method still requires a token unless a tool-name exception applies. For older versions, or requests without `Mcp-Method`, method and tool matching use a body peek limited to 16 KiB. Tool-name exceptions still require that peek even when the header is present.
...
The deprecated flag no longer allows anonymous `initialize` requests. If legacy clients need to initialize without a token, explicitly include `initialize` in `skip_token_validation.methods`.
