...
For requests declaring an SDK-known `MCP-Protocol-Version` of `2026-07-28` or later, the server checks `Mcp-Method` before reading the body. A matching method skips auth middleware buffering; a nonmatching method still requires a token unless a tool-name exception applies. This check uses the request's protocol header, including when a session exists, matching the transport's header validation. For older or unrecognized versions, or requests without `Mcp-Method`, method and tool matching use a body peek limited to 16 KiB. Tool-name exceptions still require that peek even when the header is present.

This optimization does not change the server's advertised protocol versions, currently capped at `2025-11-25`. Clients using those versions continue to use the body fallback.
...
The deprecated flag retains anonymous `initialize` access for compatibility with existing clients and also permits `server/discover`.
