...
### Authorization servers without discovery

Some authorization servers don't expose an RFC 8414 or OIDC discovery endpoint. Configure these servers under `static_servers` instead of `servers`, supplying the same two values a discovery response would otherwise provide:
...
Apollo MCP Server fetches signing keys from `jwks_uri` directly, skipping the discovery request `servers` entries perform. Caching, rate limiting, and issuer binding all behave the same as for a `servers` entry: `issuer` plays the role a discovery response's `issuer` field would, and a token's `iss` claim must match it when `issuers` is configured. `servers` and `static_servers` can be used together, and both contribute to the `authorization_servers` list this server advertises in its protected-resource metadata.

`servers` has no default, so it must still be present even when every authorization server is static:
