...
Rhai scripts run as trusted server-side code. Hooks can use request-derived data and change outbound request destinations or headers, so restrict and review script changes. See Rhai security guidance (/apollo-mcp-server/rhai-getting-started#security).
