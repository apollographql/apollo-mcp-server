...
<Tabs>
    ...
    curl -sSL https://mcp.apollo.dev/download/nix/v1.12.0-rc.1 | sh
    ```
    ...
    ```terminal showLineNumbers=false
    # Note the `v` prefixing the version number and the `-rc` suffix
    iwr 'https://mcp.apollo.dev/download/win/v1.12.0-rc.1' | iex
    ```
    </Tab>
...
```bash
# Note the `v` prefixing the version number and the `-rc` suffix
docker image pull ghcr.io/apollographql/apollo-mcp-server:v1.12.0-rc.1
```
