...
<Tabs>
    ...
    curl -sSL https://mcp.apollo.dev/download/nix/v1.11.0 | sh
    ```
    ...
    ```terminal showLineNumbers=false
    # Note the `v` prefixing the version number
    iwr 'https://mcp.apollo.dev/download/win/v1.11.0' | iex
    ```
    </Tab>
...
<Tabs>
    ...
    curl -sSL https://mcp.apollo.dev/download/nix/v1.11.0-rc.1 | sh
    ```
    ...
    ```terminal showLineNumbers=false
    # Note the `v` prefixing the version number and the `-rc` suffix
    iwr 'https://mcp.apollo.dev/download/win/v1.11.0-rc.1' | iex
    ```
    </Tab>

</Tabs>

You can configure the Apollo MCP server using a [YAML configuration file](/apollo-mcp-server/config-file). The server watches the configuration file for changes and automatically restarts when it is modified.
...
```bash
# Note the `v` prefixing the version number
docker image pull ghcr.io/apollographql/apollo-mcp-server:v1.11.0
```
...
```bash
# Note the `v` prefixing the version number and the `-rc` suffix
docker image pull ghcr.io/apollographql/apollo-mcp-server:v1.11.0-rc.1
```
