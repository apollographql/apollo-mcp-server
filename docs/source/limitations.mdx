...
- Schema validation of predefined GraphQL operation documents before exposing them as tools
- Independent enforcement of the generated JSON `inputSchema` for predefined operation arguments

The upstream GraphQL endpoint performs GraphQL document and variable validation when it executes a predefined operation. The optional `validate` tool can validate agent-supplied operations against the configured schema, but clients aren't required to call it before `execute`. The `execute` tool also accepts a new GraphQL document with each call, so it isn't bound to a previously reviewed predefined document.
