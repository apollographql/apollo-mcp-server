...
```yaml title="Config with per-operation scope requirements" {4-13}
operations:
  source: local
  paths: [./operations]
overrides:
...
Flat lists require all listed scopes. Nested lists define alternatives: each inner list is AND, and the outer list is OR.
