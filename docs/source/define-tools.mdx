...
- Union and interface fields combine selections that apply to the same concrete type with `allOf` and use `anyOf` for different selection patterns. The generator follows nested, named, and inline fragments, including inline fragments without a type condition. A fallback accepts members with no matching fragment. A response can contain fragment-selected keys from only one compatible member pattern; unrelated keys remain allowed. `__typename`, when selected, is a plain string and isn't a discriminator.
