...
In `tools/list`, predefined GraphQL operation tools appear first, alphabetically by operation name using case-sensitive lexicographic ordering (for example, `Alpha`, `Zulu`, `alpha`). With unique operation names, the same operation set retains this order across repeated calls, schema reloads, and operation reloads, regardless of source order. Operations with equal names retain their source order. Built-in and app tools follow in their existing order.

Deterministic ordering supports client-side tool-list caching and LLM prompt cache reuse. Schema reloads can still change tool descriptions or generated schemas; the guarantee applies to ordering.
...
Use [operation descriptions](#operation-descriptions) for operations you own and [config-level descriptions](#config-level-descriptions) for operations you can't edit.

### Description precedence

When more than one source is present, the first one in this order wins:

1. [Config-level descriptions](#config-level-descriptions)
2. [Operation descriptions](#operation-descriptions)
3. [`#` comments](#graphql-comments-in-operation-bodies)
4. Descriptions generated from the schema

Variable descriptions follow the same order, except that config-level descriptions don't apply to them.

### Operation descriptions

Add a description before the operation definition, as defined by the [GraphQL September 2025 specification](https://spec.graphql.org/September2025/). MCP Server uses it as the tool description.

```graphql title="operations/GetAlerts.graphql" {1-3}
"""
...
Operation descriptions take priority over `#` comments. MCP Server removes descriptions from operations, fragments, and variables before sending an operation to your GraphQL endpoint, so the endpoint doesn't need to support this syntax.

<Note>

Your router parses persisted query manifests itself. Keep using `#` comments in manifest operations until your router supports descriptions on executable definitions.

</Note>

### Variable descriptions

Add a description before an operation variable to set its input property description:
...
Variable descriptions take priority over `#` comments on the same variable and over descriptions derived from GraphQL schema arguments. A description applies to the variable as a whole, so it can't describe individual fields of an input object type. Like variable comments, variable descriptions aren't affected by `overrides.descriptions`.
...
Config-level descriptions take priority over operation descriptions, comment-based descriptions, and schema-generated tool descriptions for the same operation. Keys must exactly match operation names. An unmatched or renamed operation uses its source-derived description, and the server doesn't warn about unmatched description keys.
...
These overrides apply to tools generated from your GraphQL operations. The server's own built-in tools (`introspect`, `search`, `validate`, `execute`, and `explorer`) carry annotations that aren't configurable — an `annotations` entry named after one of them is ignored.
...
One more built-in tool sits outside the `introspection` key:

- `explorer`: returns a URL that opens a GraphQL operation in Apollo Explorer. It doesn't read the schema or call the GraphQL endpoint; it compresses the document, variables, and headers it is given into the link. Enable it with [`overrides.enable_explorer`](/apollo-mcp-server/config-file#overrides), which also requires a GraphOS graph reference.
