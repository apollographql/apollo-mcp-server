...
Apollo MCP Server loads local `.graphql` operation files and persisted query manifest bodies verbatim. Environment variable expansion using `${env.VAR}` applies only to the YAML configuration file. Do not embed credentials in operation content or expect environment placeholders there to be expanded.

</Caution>

## Understand execution and validation boundaries

For each predefined operation tool, Apollo MCP Server parses a single named GraphQL operation and stores its document separately from the model-facing tool metadata. When a client calls the tool by name, the server sends that stored document to the upstream GraphQL endpoint. Changing only the tool description or generated input schema doesn't change the stored document.

Apollo MCP Server derives the tool's JSON `inputSchema` from the operation variables and configured GraphQL schema. This schema guides MCP clients and AI models, but the server doesn't independently enforce it for predefined operation calls. The server also doesn't validate a predefined operation against the configured GraphQL schema before exposing it as a tool. The upstream GraphQL endpoint performs GraphQL document and variable validation during execution, so test and review predefined operations before deploying them.

The optional [`validate` tool](#introspection-tools) validates an agent-supplied operation against the current schema. It isn't an automatic prerequisite for `execute`, and it doesn't prevalidate predefined operation tools.
...
Apollo MCP Server polls operation collections containing up to 100 operations. A collection with more than 100 operations is loaded, but subsequent polling is disabled. If a later update increases the collection beyond 100 operations, that update is applied and polling then stops.
...
Hot-reloaded operation and schema sources can change tool names, descriptions, inputs, and executable documents without an automatic security diff or approval step. Restrict source write access and [review the live tool definitions](/apollo-mcp-server/best-practices#protect-tool-definitions-from-poisoning) after changes.
...
`overrides.descriptions` controls only the top-level tool description. It doesn't change or pin the operation name, input property names or descriptions, annotations, or executable GraphQL document.

### Config-level scope requirements

To restrict which OAuth scopes are required to call a specific tool, add a `required_scopes` map under the `overrides` config key. When a token lacks the required scopes, the server returns HTTP 403 and the client can re-authorize with the precise scopes needed. See [Per-operation scope requirements](/apollo-mcp-server/auth#per-operation-scope-requirements) for details.

```yaml title="Config with per-operation scope requirements" {4-9}
operations:
  source: local
  paths: [./operations]
overrides:
...
Keys must exactly match the MCP tool name sent in `tools/call`. Only listed tools receive an additional scope check; an empty list or an absent entry leaves the tool governed by the global scope requirement alone. The server accepts unknown keys without warning, so review this map whenever tools are added, removed, or renamed.
...
- `validate`: validates a GraphQL operation against the configured schema without executing it. Operations should be validated prior to calling the `execute` tool.
...
Unlike predefined operation tools, `execute` accepts the GraphQL document from each tool call and isn't pinned to a previously reviewed document. It checks GraphQL syntax, rejects subscriptions, and applies [`mutation_mode`](/apollo-mcp-server/config-file#mutation-modes), but it doesn't automatically validate fields, arguments, or variable types against the configured schema. Enabling `validate` doesn't force clients to call it before `execute`; the upstream GraphQL endpoint ultimately validates any operation that is forwarded.

Limit `execute` and mutation access to use cases that need dynamic operations, and apply authorization and demand controls at the GraphQL API. See [Operation validation and execution](/apollo-mcp-server/limitations#operation-validation-and-execution) for the complete boundary.
