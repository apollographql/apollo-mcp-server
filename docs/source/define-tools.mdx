...
- A union field with inline or named fragments becomes an `anyOf` of object schemas for the selected type conditions. If some union members have no matching fragment, a fallback accepts their objects while excluding fields selected only by the other fragments. For an interface field, fields from inline fragments are merged into a single object schema. `__typename`, when selected, is a plain string and isn't a discriminator.
