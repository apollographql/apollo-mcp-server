...
Tool descriptions help AI models understand when and how to use each tool. If no custom description is provided, the MCP Server automatically generates one from the GraphQL schema's field and type descriptions.

### GraphQL comments in operation bodies

Embed `#` comments before the operation definition. The MCP Server extracts leading comments and uses them as the tool description.
