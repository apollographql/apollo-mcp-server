...
- `validate`: validates a GraphQL operation against the configured schema without executing it. This checks schema conformance, not whether the operation matches the user's intent or is safe to run. Operations should be validated prior to calling the `execute` tool.
