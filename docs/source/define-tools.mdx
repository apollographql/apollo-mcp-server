...
MCP tool annotations are hints that help AI clients understand tool behavior. By default, queries are annotated with `read_only_hint: true` and mutations with `destructive_hint: true`. To customize these hints or add additional ones like `idempotent_hint` or `open_world_hint`, add an `annotations` map under the `overrides` config key.
...
Each annotation field is optional. Only the fields you specify override the auto-detected defaults; unset fields keep their original values.

| Field              | Type     | Description                                                        |
| :----------------- | :------- | :----------------------------------------------------------------- |
| `title`            | `String` | A human-readable title for the tool                                |
| `read_only_hint`   | `bool`   | If true, the tool does not modify its environment                  |
| `destructive_hint` | `bool`   | If true, the tool may perform destructive updates                  |
| `idempotent_hint`  | `bool`   | If true, calling the tool repeatedly has no additional effect      |
| `open_world_hint`  | `bool`   | If true, the tool may interact with systems outside its own domain |
