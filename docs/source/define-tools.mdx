...
The MCP Server supports hot reloading of the GraphOS operation collection, so it picks up changes from GraphOS without restarting. The MCP Server polls GraphOS for changes periodically, so expect up to **30–60 seconds** before new or modified operations appear as tools. See [Operation source polling](/apollo-mcp-server/config-file#operation-source) for details.
...
#### Tool descriptions for collection operations

Add a `#` comment on the line immediately before the `query` or `mutation` keyword — the MCP Server uses it as the tool description.

Tool descriptions help AI models understand when and how to use each tool. For operations loaded from a GraphOS operation collection, the MCP Server derives tool descriptions from **leading `#` comments** placed immediately before the `query` or `mutation` keyword in the operation body.

<Note>
Only leading comments (lines starting with `#` that appear **before** the operation definition) are used as tool descriptions. Comments placed **inside** the operation body — for example, above or beside field selections — are ignored.
</Note>

The following examples show correct and incorrect comment placement.

**✅ Correct placement — leading comment becomes the tool description:**
```graphql title="Good: comment before the operation definition"
# Retrieve a single product by its ID, including pricing and inventory
query GetProduct($productId: ID!) {
  product(id: $productId) {
    id
    name
...
**❌ Incorrect placement — inline and body comments are ignored:**
```graphql title="Bad: comments inside the body are NOT used"
query GetProduct($productId: ID!) {
  # Fetch product details  ← ignored by the MCP Server
  product(id: $productId) {
    id
    name
    price
...
<Note>
If no leading comment is present, the MCP Server automatically generates a description from the schema's field and type descriptions.
</Note>
...
#### Tool descriptions for manifest operations

Tool descriptions help AI models understand when and how to use each tool. You can provide descriptions for operations in a persisted query manifest using either of the following methods.

**Using GraphQL comments in the operation body** (recommended): Embed `#` comments before the operation definition in the `body` field. The MCP Server extracts leading comments and uses them as the tool description.

```json title="Manifest with comment-based descriptions" {7}
{
...
**Using config-level descriptions**: Add a `descriptions` map to your `operations` config that maps operation names to tool descriptions. These descriptions override auto-generated descriptions for the matching operations.

```yaml title="Config with tool descriptions for manifest operations" {4-5}
operations:
  source: manifest
  path: <PATH/TO/persisted-queries-manifest.json>
  descriptions:
...
If neither config-level descriptions nor leading comments are provided, the MCP Server automatically generates a description from the GraphQL schema's field and type descriptions.
