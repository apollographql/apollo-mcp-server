...
To restrict which OAuth scopes are required to call a specific tool, add a `required_scopes` map under the `overrides` config key. When a token lacks the required scopes, the server returns HTTP 403 and the client can re-authorize with a compatible scope set. See [Per-operation scope requirements](/apollo-mcp-server/auth#per-operation-scope-requirements) for details.
