...
| `csp.baseUriDomains`<br/>(optional)             | Approved origins for base URI         | `_meta.ui.csp.baseUriDomains` (MCP Apps only)                                                     | MCP Apps only. Approved origins for the `base-uri` CSP directive. Must include protocol (`https://`)                                                                                                                                                                                                    |
...
Since `.application-manifest.json` is fully generated, you can safely add it to your `.gitignore`.
...
All app operations are subject to [`overrides.mutation_mode`](/apollo-mcp-server/config-file#mutation-modes). `explicit` and `all` allow mutation operations from an app manifest because they are predefined. With `none`, Apollo MCP Server rejects an app manifest containing a mutation instead of partially loading your app.
...
| -------------- | -------------------------------- | -------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `name`         | Tool name                        | No       | MCP tool `name`                       | Defaults to the GraphQL operation name if not provided. Must be unique within the app. An anonymous operation with a bare `@tool` directive fails validation. When multiple `@tool` directives are present on the same operation, `name` must be provided explicitly on each.                                                         |
| `description`  | Tool description                 | No       | MCP tool `description`                | Defaults to the GraphQL operation description if not provided. App description is prefixed to tool description if present. An operation without a description using a bare `@tool` directive fails validation. When multiple `@tool` directives are present on the same operation, `description` must be provided explicitly on each. |
| `extraInputs`  | Additional tool inputs           | No       | Extra inputs in MCP tool schema       | Array of objects with `name`, `description`, and `type` ("string", "number", or "boolean"). These inputs aren't part of the GraphQL operation but are provided by the host                                                                                                                                                            |
| `extraOutputs` | Additional free-form output data | No       | Extra outputs in manifest tool config | Object for writing free-form data to the manifest's tool config. Use this to pass additional metadata that should be included in the tool's manifest entry.                                                                                                                                                                           |
...
The `@private` directive hides specified fields from the LLM while still making them accessible to your app's UI code, which prevents sensitive data from being exposed in AI responses.

**Usage**: Add `@private` to individual fields within a GraphQL operation.

| Element    | What it is                      | Where it maps to                     | Notes                                                                                             |
| ---------- | ------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------- |
| `@private` | Field visibility control marker | Field marked private in the manifest | The response excludes fields from the LLM-visible response, but they remain available in your app |
