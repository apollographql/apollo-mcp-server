...
- **Downstream GraphQL Calls**: The `execute` tool and custom operation tools create child spans for their outgoing `reqwest` HTTP calls, capturing the duration of the downstream request. The `traceparent`, `tracestate`, and `baggage` headers are propagated automatically. Baggage is request-scoped context, not identity, and is not automatically copied onto exported span attributes.
