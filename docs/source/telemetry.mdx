...
| --------------------------- | --------------------------------------------------------------------------------------- |
| `http.request.method`       | The HTTP method, for example `POST`.                                                     |
...
| `apollo.mcp.session_id`     | The MCP session ID, from the response on `initialize` and from the request after that.   |

The server sets the span status to error for a 5xx response and leaves it unset otherwise, which is what the conventions require. The span stays open until the response body finishes streaming, so its duration covers tool execution on a streamed response.
