...
- **Downstream GraphQL Calls**: Apollo MCP Server's `execute` tool and custom operation tools create child spans for their outgoing `reqwest` HTTP calls, capturing the duration of the downstream request. The `traceparent`, `tracestate`, and `baggage` headers propagate automatically. Baggage is request-scoped context, not identity. Apollo MCP Server does not automatically copy baggage onto exported span attributes.
