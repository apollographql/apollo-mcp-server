...
The GraphQL operations can be configured from persisted queries, which are predefined lists of operations registered with and maintained by a graph. The operations can also be determined by AI introspecting your graph schema.
...
* Tool Generation: Apollo MCP Server creates MCP tools from your GraphQL operations using:
    ...
    * GraphOS Operation Collections: Predefined operation lists maintained in GraphOS
    * Persisted Query Manifests: Predefined operation lists loaded locally or from GraphOS
    * Schema Introspection: Dynamic operation discovery for flexible AI exploration

* Structured Execution: When invoked, the server sends a structured GraphQL HTTP request to your API endpoint using the configured headers and authentication.
* Existing Infrastructure: Your GraphQL API handles requests normally, with Apollo MCP Server acting as a GraphQL-to-MCP bridge rather than requiring any changes to your graph.

This design lets you choose which GraphQL capabilities to expose to AI while continuing to enforce data access and security policies at your GraphQL API.
...
**🎯 Typed Execution**: GraphQL operations declare their fields, arguments, and response shapes. The GraphQL API validates each request against its schema before resolver execution. This makes the request contract explicit, but doesn't determine which operation an AI agent should call or in what sequence.

**🛡️ Policy Reuse**: Authentication, authorization, and data-access policies implemented by your GraphQL API continue to apply when it receives operations from Apollo MCP Server, including operations that span multiple underlying APIs or microservices.
...
- **🔒 Govern AI access**. By using predefined persisted queries and restricting who can update operation sources, developers can govern which data and operations AI clients can access. The GraphQL API continues to enforce its authentication, authorization, and data-access policies.
