...
You can configure GraphQL operations from persisted queries, which are predefined lists of operations registered with and maintained by a graph. AI can also determine operations by introspecting your graph schema.
...
This design enables you to expose specific GraphQL capabilities to AI while continuing to enforce data access and security policies at your GraphQL API.
...
- **🔒 Govern AI access**. By adopting predefined persisted queries and restricting who can update operation sources, developers can govern which data and operations AI clients can access. The GraphQL API continues to enforce its authentication, authorization, and data-access policies.
