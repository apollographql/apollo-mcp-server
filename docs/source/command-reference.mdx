...
A prebuilt container containing the Apollo MCP Server is built with every release at `ghcr.io/apollographql/apollo-mcp-server`.
...
- **SSE Transport on port 5000**: Make sure to export container port 5000 for SSE connections to
