...
Each variant will have it's own default MCP Tools Collection, but you can specify any shared collection by using `operations` with `operations.source: collection`.
...
You can set the persisted query manifest file for the MCP Server by using the `operations` option. The MCP Server supports hot reloading of persisted query manifests, so changes to manifests are applied without restarting.
...
apollo-mcp-server <path to the above config>
```
...
docker run \
  ...
  -p 5000:5000 \
  -v <path to the above config>:/config.yaml \
