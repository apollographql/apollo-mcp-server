...
1. For the **Identifier** field, Auth0 recommends using a URL. This identifier is used in the MCP server configuration later as the `audience` property. For this guide, use `http://127.0.0.1:8000/mcp`.
...
1. Navigate to your dashboard **Settings**.
   ...
   1. Toggle on **Dynamic Client Registration (DCR)** to enable [dynamic client registration](https://auth0.com/docs/get-started/applications/dynamic-client-registration#enable-dynamic-client-registration).
...
1. Replace the `<AUTH0 DEFAULT AUDIENCE>` with the matching `Identifier` you set when creating the Auth0 API. In this guide, you used `http://127.0.0.1:8000/mcp`.
