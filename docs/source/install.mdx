...
docker image pull ghcr.io/apollographql/apollo-mcp-server:v0.9.0
```
...
docker image pull ghcr.io/apollographql/apollo-mcp-server:v0.9.0-rc.1
```
...
```bash
# Note the `v` prefixing the version number
iwr 'https://mcp.apollo.dev/download/win/v0.9.0' | iex
```
