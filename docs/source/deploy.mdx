...
subtitle: Deployment using the Apollo GraphOS Operator or Docker containers, when to choose which option, and production considerations
---

To deploy Apollo MCP Server in your production environment, use the [Apollo GraphOS Operator](#deploy-on-kubernetes-with-the-apollo-graphos-operator-recommended-for-kubernetes) if you're running on Kubernetes. Otherwise, use the [Apollo Runtime Container](#apollo-runtime-container-recommended-for-non-kubernetes-deployments). A [standalone Apollo MCP Server container](#standalone-apollo-mcp-server-container) is also available if you need to run it separately from your GraphQL API.

## Deploy on Kubernetes with the Apollo GraphOS Operator (Recommended for Kubernetes)

If you run Apollo Router on Kubernetes using the [Apollo GraphOS Operator](/apollo-operator/), use it to deploy Apollo MCP Server too. The operator manages the Deployment, Service, ConfigMap, and ephemeral graph API key Secret for you, and can wire an MCP Server directly to an existing Router it also manages.

<Note>

Deploying Apollo MCP Server this way requires Apollo GraphOS Operator v1.5.0 or later, and an Apollo GraphOS Developer, Standard, or Enterprise plan.

</Note>

The operator forces `transport.stateful_mode: false`, so the session-affinity requirements described under [Load Balancing & Session Affinity](#load-balancing--session-affinity) don't apply to operator-managed deployments — its plain Kubernetes Service works without any special load balancer configuration.

For setup instructions, see the [Apollo GraphOS Operator documentation](/apollo-operator/).

## Apollo Runtime Container (Recommended for non-Kubernetes deployments)
...
|-----------------------------------------------|------------------------------|-----------------------------------------------------------------------------------------------------------|
| Kubernetes + GraphOS-managed graph            | Apollo GraphOS Operator      | Manages the Deployment/Service/ConfigMap/Secret lifecycle for you                                         |
| New GraphQL + MCP deployment (non-Kubernetes) | Apollo Runtime Container     | Single container, easier to manage, optimized performance                                                 |
| GraphOS-managed graph (non-Kubernetes)        | Apollo Runtime Container     | Automatic sync for schema and persisted queries, unified telemetry                                        |
...
For Apollo GraphOS Operator-managed deployments:

- Set `spec.replicas` on the `McpServer` resource, or attach a Kubernetes `HorizontalPodAutoscaler` directly to it — the operator exposes a `scale` subresource, so the `McpServer` itself can be the autoscaling target
- Because the operator forces `transport.stateful_mode: false`, scaling out requires no session-affinity configuration on the Service it manages
